...
resource "aws_iam_role" "petra_kops_role" {
  name = "Petra-KOPS-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17",
    Statement = [{
      Effect = "Allow",
...
resource "aws_iam_role_policy_attachment" "attach_kops_policy_to_petra_role" {
  role       = aws_iam_role.petra_kops_role.name
